...
class PasienController extends Controller
{
    ...
    public function destroy(string $id): RedirectResponse
    {
        try {
            ...
            // Check for related medical data (appointments, examinations, etc.)
            $hasAppointments = $pasien->daftarPoli()->exists();
            $hasExaminations = $pasien->periksa()->exists();

            if ($hasAppointments || $hasExaminations) {
                return redirect()->route('admin.pasien.index')
                    ->with('warning', 'Pasien tidak dapat dihapus karena memiliki riwayat pendaftaran atau pemeriksaan.');
            }
            ...
            return redirect()->route('admin.pasien.index')
                ->with('error', 'Gagal menghapus data pasien. Silakan coba lagi.');
        }
    }
}
