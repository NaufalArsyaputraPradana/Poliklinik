...
class DokterController extends Controller
{
    ...
    public function destroy(User $dokter): RedirectResponse
    {
        try {
            ...
            // Check for related data (appointments, schedules, etc.)
            $hasSchedules = $dokter->jadwalPeriksa()->exists();
            $hasPatients = $dokter->periksa()->exists();

            if ($hasSchedules || $hasPatients) {
                return redirect()->route('admin.dokter.index')
                    ->with('warning', 'Dokter tidak dapat dihapus karena masih memiliki jadwal atau riwayat pemeriksaan.');
            }
